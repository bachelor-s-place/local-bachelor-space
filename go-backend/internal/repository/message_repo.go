...
// MarkRead appends the userID to the read_by array for every message in the squad
// the user has not already read (a "mark all as read" action). The
// NOT (... = ANY(read_by)) guard plus row-level locking under READ COMMITTED makes
// concurrent calls for the same user idempotent (no duplicate entries).
