...
// GetAllVerifications returns all verifications joined with property title, newest first.
// Used by the admin verification queue page.
func (r *VerificationRepo) GetAllVerifications(ctx context.Context) ([]map[string]interface{}, error) {
	const query = `
		SELECT v.id, v.property_id, COALESCE(p.title, '') AS property_title,
		       v.verification_type AS type, v.status, v.notes, v.created_at
	...
	for rows.Next() {
		var id, propertyID, propertyTitle, vType, status string
		var notes *string
		var createdAt interface{}
		if err := rows.Scan(&id, &propertyID, &propertyTitle, &vType, &status, &notes, &createdAt); err != nil {
			return nil, err
		}
