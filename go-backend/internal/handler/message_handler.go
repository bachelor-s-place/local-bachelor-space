...
func (h *MessageHandler) GetMessages(w http.ResponseWriter, r *http.Request) {
	...
	// Clamp to the same bounds the repo enforces, so the next_cursor check below
	// (len(messages) == limit) stays correct — otherwise a large limit silently
	// disables pagination and older messages become unreachable.
	if limit <= 0 || limit > 50 {
		limit = 30
	}
...
func (h *MessageHandler) SendMessage(w http.ResponseWriter, r *http.Request) {
	...
	sanitizedContent := p.Sanitize(input.Content)

	msg, err := h.repo.CreateMessage(r.Context(), squadID, userID, sanitizedContent, contentType)
	if err != nil {
		respond.Error(w, apierror.Internal(err.Error()))
		return
	}
