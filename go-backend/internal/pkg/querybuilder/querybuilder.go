...
// MaxLimit caps how many rows any query built here can request, so a caller can
// never (accidentally or via user input) ask for an unbounded result set.
const MaxLimit = 100

// OrderBy sets the ORDER BY clause. Subsequent calls overwrite the previous value.
//
// SECURITY: the clause is concatenated verbatim (ORDER BY columns cannot be bound
// parameters in SQL). NEVER pass user-supplied input here. If a sort param must be
// user-controlled, map it through a fixed allow-list of "column direction" strings
// in the caller and pass only the validated result.
func (b *Builder) OrderBy(clause string) *Builder {
	b.orderBy = clause
	return b
}

// Limit sets the LIMIT value as a bound parameter, clamped to [0, MaxLimit].
...
// Offset sets the OFFSET value as a bound parameter, clamped to be non-negative.
