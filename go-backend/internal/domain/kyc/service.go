...
type Repository interface {
	...
	ListPending(ctx context.Context) ([]KYCWithUser, error)
}
...
func (s *Service) SubmitKYC(ctx context.Context, userID string, input SubmitKYCInput) error {
	...
	k := &LandlordKYC{
		...
		PANEncrypted:     &encPAN,
		// Submissions start as 'pending' and MUST be approved by an admin via
		// ReviewKYC before the landlord is treated as verified. Never auto-verify.
...
func (s *Service) ListPending(ctx context.Context) ([]KYCWithUser, error) {
