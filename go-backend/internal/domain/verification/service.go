...
type Repository interface {
	...
	GetAllVerifications(ctx context.Context) ([]map[string]interface{}, error)
}
...
func (s *Service) ReviewVerification(ctx context.Context, verificationID string, adminID string, input UpdateVerificationInput) error {
	...
	// State guard: only a pending verification can be reviewed. This prevents an
	// already-approved/rejected record from being re-flipped (which previously let a
	// property be promoted and then have its underlying verification silently changed).
	if v.Status != StatusPending {
		return ErrAlreadyReviewed
	}

	// Dual-control (BR): the admin who initiated a verification cannot also be the one
	// who approves/rejects it. Two distinct admins must be involved.
	if v.AdminID != nil && *v.AdminID == adminID {
...
// ListVerifications returns all verifications for the admin queue.
func (s *Service) ListVerifications(ctx context.Context) ([]map[string]interface{}, error) {
	return s.repo.GetAllVerifications(ctx)
}
