...
var (
	...
	ErrAlreadyReviewed         = errors.New("verification: already reviewed")
	ErrSelfReview              = errors.New("verification: initiator cannot review their own verification")
)
...
func ToAPIError(err error) *apierror.APIError {
	...
	case errors.Is(err, ErrAlreadyReviewed):
		return apierror.Conflict("this verification has already been reviewed")
	case errors.Is(err, ErrSelfReview):
		return apierror.Forbidden("the admin who initiated a verification cannot also review it")
	default:
