...
> **Project:** NameNotDecided (product name: BachelorsSpace)
> **Go Module:** `bachelors_space`
> **Date:** April 2026
...
| 7 | Squad System | ✅ Completed (`GET /squads/mine` added May 2026) | 3, 5 |
| 8 | Transactions & Payments | 🔲 Not started | 7 |
| 9 | Messages | ✅ Completed | 7 |
| 10 | Notifications | ✅ Completed | 2–9 |
...
| File | Purpose |
|------|---------|
| `go.mod` | Module `bachelors_space`, Go 1.22 |
...
| `GET` | `/api/v1/properties/{id}/applications` | Required (landlord) | View squads awaiting landlord approval |
| `PUT` | `/api/v1/squads/{id}/application_status` | Required (landlord) | Landlord approves or rejects squad application |
...
- **Landlord-First Approval Flow (Option A):** Accepting a proposal puts the squad in `pending_landlord_approval`. If approved by the landlord, it moves to `payment_pending`. Only then can tokens be paid to reach `locked`.
...
| Squad application received | `squad_application_received` | ✅ |
| Squad application approved | `squad_application_approved` | ✅ |
| Squad application rejected | `squad_application_rejected` | ❌ |
...
# Database
DATABASE_URL=postgres://user:pass@localhost:5432/bachelorsspace_db?sslmode=disable
